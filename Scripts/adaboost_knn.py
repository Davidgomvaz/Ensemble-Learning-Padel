...
def base_classifier():
    ...
    y_pred = knn_clf.predict(X_test) # predicciones con datos de test
...
def best_params():
    ...
    model = model_selection.GridSearchCV(estimator= AdaBoostClassifier(estimator=knn_clf),
...
# Funcion para encontrar los mejores parametros
def iter_class(hip1,hip2):
    knn_clf=base_classifier()
    for i in hip1:
        for j in hip2:
            ada_clf=AdaBoostClassifier(estimator=knn_clf,n_estimators=i,learning_rate=j)
...
def adaboost_class():
    knn_clf=base_classifier()
    ada_clf = AdaBoostClassifier(estimator=knn_clf,n_estimators=200,learning_rate=0.5)
    ada_clf.fit(X_train, y_train)
    y_pred = ada_clf.predict(X_test)
