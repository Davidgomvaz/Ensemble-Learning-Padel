...
def base_classifier():
    svm_clf = svm.SVC(C=5, decision_function_shape='ovr', kernel='rbf', probability=True)
    svm_clf.fit(X_train, y_train) # realizamos el entrenamiento
    y_pred = svm_clf.predict(X_test) # predicciones con datos de test
...
def best_params():
    ...
    model = model_selection.GridSearchCV(estimator= AdaBoostClassifier(estimator=svm_clf),
...
def iter_clf():
    ...
    svm_clf=base_classifier()
    for i in range(5):
        bag_clf = AdaBoostClassifier(estimator=svm_clf,n_estimators=250,learning_rate=0.01)
    ...
    pyplot.title('Resultado AdaBoost con SVM')
    pyplot.ylabel('Precisión(%)')

def param_results(hip1,hip2):
    svm_clf=base_classifier()
    for i in hip1:
        for j in hip2:
            ada_clf=AdaBoostClassifier(estimator=svm_clf,n_estimators=i,learning_rate=j)
            ada_clf.fit(X_train, y_train)
            y_pred = ada_clf.predict(X_test)
            print( i, j, ada_clf.__class__.__name__, accuracy_score(y_test, y_pred))
    pass

# Funcion para definir y entrenar el clasificador por Adaboost
def adaboost_class():
    svm_clf=base_classifier()
    ada_clf = AdaBoostClassifier(estimator=svm_clf,n_estimators=250,learning_rate=0.01)
...
# best_params()
# hip1=[50,100,250,500]
# hip2=[0.01,0.05,0.1,0.5,1]
# param_results(hip1,hip2)
# iter_clf()
adaboost_class()
