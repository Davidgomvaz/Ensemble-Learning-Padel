# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 12:31:40 2023
...
def best_params():
    knn_clf=base_classifier()
    param_grid={'estimator': [knn_clf],
                'n_estimators': [10,50,100,250,500], #10,50,100,250,500
                'max_samples': [100,250,500,750,1000], #100,250,500,700
                'max_features': [25,50,100,200],#25,50,100,200
                'bootstrap': [True]}
...
def param_results(n_estimators,max_feature):
    ...
    for i in n_estimators:
        for j in max_feature:
            past_clf = BaggingClassifier(estimator=knn_clf, n_estimators=i,max_features=j, bootstrap=False,n_jobs=-1)
...
def iter_clf():
    ...
    knn_clf=base_classifier()
    for i in range(10):
        bag_clf = BaggingClassifier(estimator=knn_clf, n_estimators=500, bootstrap=False, max_features=50, n_jobs=-1)
    ...
    pyplot.title('Resultado Pasting con KNN')
