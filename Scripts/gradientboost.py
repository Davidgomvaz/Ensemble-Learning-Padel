# -*- coding: utf-8 -*-
"""
Created on Sun Apr 30 12:20:50 2023
...
def base_classifier():
    dt_clf = DecisionTreeClassifier(max_depth=40, min_samples_leaf=1, min_samples_split=4, criterion="entropy")
...
param_grid={'n_estimators': [50,100,250],
            'learning_rate': [00.1,0.05,0.1,0.5,1]}

# Funcion para encontrar los mejores parametros
def best_params():
    dt_clf=base_classifier()
    model = model_selection.GridSearchCV(estimator= GradientBoostingClassifier(),
...
def param_results(hip1,hip2):
    for i in hip1:
        for j in hip2:
            ada_clf=GradientBoostingClassifier(n_estimators=i,learning_rate=j,max_depth=20, min_samples_leaf=1, min_samples_split=4)
...
def iter_clf():
    result=list()
    for i in range(5):
        ada_clf = GradientBoostingClassifier(n_estimators=500,learning_rate=0.5,max_depth=20, min_samples_leaf=1, min_samples_split=4)
        ada_clf.fit(X_train, y_train)
        y_pred = ada_clf.predict(X_test)
    ...
    pyplot.title('Resultado Gradient Boosting')
    pyplot.ylabel('Precisión(%)')

# Funcion para definir y entrenar el clasificador Adaboost
def gradientbost_class():
    dt_clf=base_classifier()
    ada_clf = GradientBoostingClassifier(n_estimators=500,learning_rate=0.5,max_depth=20, min_samples_leaf=1, min_samples_split=4)
