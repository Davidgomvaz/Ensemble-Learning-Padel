# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 01:53:16 2023
...
def iter_clf():
    ...
    result=list()
    for i in range(5):
        ada_clf = AdaBoostClassifier(estimator=dt_clf,n_estimators=250,learning_rate=0.1)
...
def param_results(hip1,hip2):
    ...
    for i in hip1:
        for j in hip2:
            ada_clf=AdaBoostClassifier(estimator=dt_clf,n_estimators=i,learning_rate=j)
            ada_clf.fit(X_train, y_train)
            y_pred = ada_clf.predict(X_test)
