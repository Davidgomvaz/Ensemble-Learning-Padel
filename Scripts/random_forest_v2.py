# -*- coding: utf-8 -*-
"""
Created on Mon Mar 27 10:58:57 2023
...
param_grid={'n_estimators': [10,50,100,250,500], #10,50,100,250,500
            'max_samples': [None,1000], #100,250,500,
            'max_depth': [20],
            'criterion': ['entropy'],
            'min_samples_split': [4],
            'min_samples_leaf': [1],
...
def best_params_rf():
    ...
    model = model_selection.GridSearchCV(estimator= RandomForestClassifier(),
...
def iter_clf():
    result=list()
    for i in range(10):
        rf_clf=ExtraTreesClassifier(n_estimators=500, bootstrap=False, criterion='entropy',max_depth=20, min_samples_split=4,min_samples_leaf=1,n_jobs=-1)
...
def param_results(n_estimators):
    ...
    for i in n_estimators:
        rf_clf = RandomForestClassifier(n_estimators=i, bootstrap=False, criterion='entropy',max_depth=20, min_samples_split=4,min_samples_leaf=1,n_jobs=-1)
        rf_clf.fit(X_train, y_train)
        y_pred = rf_clf.predict(X_test)
...
def rt_classifiers():
    dt_clf=base_classifier()
    # rf_clf=RandomForestClassifier(n_estimators=500, bootstrap=False, criterion='entropy',max_depth=20, min_samples_split=4,min_samples_leaf=1,n_jobs=-1)
    ert_clf=ExtraTreesClassifier(n_estimators=500, bootstrap=False, criterion='entropy',max_depth=20, min_samples_split=4,min_samples_leaf=1,n_jobs=-1)
